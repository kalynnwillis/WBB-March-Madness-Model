...
We use the Bradley-Terry model as a probabilistic framework that models pairwise comparisons, which in this case are the pairwise game outcomes (which women’s NCAA team will win). For teams I and J with latent strength parameters $\lambda_i$ and $\lambda_j$, the probability that Team I will beat Team J is the following:

$$
P(\text{team } i \text{ beats team } j) = \frac{e^{\lambda_i}}{e^{\lambda_i} + e^{\lambda_j}} = \frac{1}{1 + e^{-(\lambda_i - \lambda_j)}}
$$

To explain the above model, lambda is an estimated team strength parameter that will linearly affect a team’s log-odds of winning. For example, for Team I and Team J, $\lambda_i$ represents the log-strength of Team I. The larger the lambda, the stronger the team. The win probability between Team I and J depends on the difference of their lambda values (the larger the difference, the more probable it is that one team will win over the other). 

### Synthetic Tournament Seeds

For the construction of our model, we used synthetic tournament seeds based on the team strength generated by Bradley-Terry lambda values. We do not analyze historical Women’s March Madness brackets, but use the regular season games to predict how teams seeded 8 - 12 will perform in the March Madness tournament.

We assigned seeds 1 - 16 to the top 64 teams generated by our Bradley-Terry latent strength parameters across the five seasons. It is important to note that this excludes the actual NCAA committee-selected seeds, which could be affected by varying factors including geographic restrictions, conference tournament results, or any subjective decisions. This matters for our analysis because the seed-specific probabilities (including those 8 - 12) are assigned by the lambda values rather than the actual historical rankings. 

We chose this approach because it allows for an unbiased analysis of predictions of the March Madness tournament, demonstrating the results if there were “perfect seeding” in terms of how good the Bradley-Terry model found each team to be.


### Covariates Considered and Model Comparison

Our model adds a home team indicator variable to control for potential advantage of playing at home. We explored implementing further covariates to enhance our base Bradley-Terry model. For instance, each team’s lambda may vary by a team’s offensive and defensive abilities. We ran models with a defensive rating (points allowed per game), offensive rating (points scored per game), point differential per game and the margin of victory to see if any of these models would be more robust than our base Bradley-Terry model. 
...
The covariates are redundant because the team strength (λ) correlates strongly with the performance metrics. For exmaple, teams with a higher offensive rating that score more points, will win more games, which increases their generated λ. Implementing these covariates into the model doesn't add new information. 
...
After fitting the model, we collected our data using the wehoop R package (which includes comprehensive game-level data from ESPN) and pulled the Women’s NCAA Basketball data for 5 total seasons: 2019 and 2021-2024. We chose to exclude 2020 due to potential disruptions from COVID-19. In total, we analyzed 372 teams with 10808 total games. Throughout the 5 seasons there were 20 teams with 8 - 12 seeds.
