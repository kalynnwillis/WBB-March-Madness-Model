...
8-12 to reach the Sweet Sixteen. Our simulations show an average of 2.21
teams (range 0-7), with most tournaments having 1-3 mid-tier seeds
advance. The 2023-2024 drought (zero advancing) has about a \textbf{7\%
chance} of happening---genuinely unusual, occurring naturally about once
every \textbf{15 years}, not every tournament cycle.

When a mid-tier seed breaks through to the Sweet Sixteen, what happens
next? About one in seven reach the Final Four, and only one in
fifty-three win the championship. Making the Sweet Sixteen is the
hardest part; once there, they're competitive (45\% reach Elite 8) but
...
\tightlist
\item
  \textbf{Expected 8-12 seeds in Sweet 16:} 2.21 teams (simulation
  average, range 0-7)
\item
...
\item
  \textbf{Probability of zero 8-12 seeds in Sweet 16:} 6.84\% (about
  1-in-15 tournaments)
\end{itemize}
...
zero mid-tier seeds advancing was even rarer---a 1-in-15 occurrence that
...
This analysis uses \textbf{synthetic tournament seeds} based on team
strength (Bradley-Terry λ values), not actual NCAA selection committee
seeds. Results show what would happen if seeds perfectly matched team
quality---a ``what-if'' scenario. Real NCAA tournaments involve
committee decisions that may over- or under-seed teams relative to their
true strength. To predict an actual tournament, you would need to use
the real NCAA bracket. See Section 4.3 for detailed discussion of this
limitation.
...
\subsection{1.4 Historical Context and
Scope}\label{historical-context-and-scope}
...
\textbf{Important Note on Scope:} This analysis uses \textbf{synthetic
tournament seeds} based on team strength (Bradley-Terry λ values) to
understand the \emph{expected} dynamics of mid-tier seeds in
tournaments. We do not analyze actual historical NCAA tournament
brackets. Instead, we ask: ``Given team strengths derived from regular
season play, what should we expect from teams at the 8-12 seed strength
level?'' This approach provides insight into tournament dynamics under
optimal seeding, though real NCAA committee decisions may deviate from
pure strength-based rankings. See Section 2.2 and Section 4.3 for
detailed discussion of this methodological choice and its implications.
...
\subsubsection{Covariates Considered and Model
Comparison}\label{covariates-considered-and-model-comparison}

Following Professor Deshpande's recommendation, we compared the
predictive performance of multiple models rather than relying solely on
AIC.

\textbf{Tested covariates:} - \textbf{Offensive rating}: Points scored
per game - \textbf{Defensive rating}: Points allowed per game -
\textbf{Net rating}: Point differential per game - \textbf{Margin of
victory (MOV)}: Average winning margin

\textbf{Models Evaluated:}

We fit five models and compared their \textbf{predictive performance} on
a holdout test set (2024 season), training on 2019, 2021-2023:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  \textbf{Base Model:} \texttt{team\ +\ home\_advantage}
\item
  \textbf{Net Rating Model:}
  \texttt{team\ +\ home\_advantage\ +\ net\_diff}
\item
  \textbf{Off/Def Model:}
  \texttt{team\ +\ home\_advantage\ +\ off\_diff\ +\ def\_diff}
\item
  \textbf{MOV Model:} \texttt{team\ +\ home\_advantage\ +\ mov\_diff}
\item
  \textbf{Combined Model:}
  \texttt{team\ +\ home\_advantage\ +\ net\_diff\ +\ mov\_diff}
\end{enumerate}
...
\bottomrule\noalign{}
\endlastfoot
Base & \textbf{69.89\%} & \textbf{0.5779} & \textbf{0.1970} &
\textbf{0.0092} \\
...
All models show \textbf{identical predictive performance} across all
metrics. This is not a computational error---it reflects genuine
redundancy.

\begin{figure}

{\centering \includegraphics[width=33.33in]{../results/figures/18_lambda_comparison} 

}

\caption{Team abilities (lambda) from base model vs. covariate model show strong correlation (r = 0.85), but predictions are identical because relative team differences remain unchanged.}\label{fig:lambda-comparison}
\end{figure}

\textbf{Why are covariates redundant?}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{High correlation:} Team strength (λ) correlates strongly with
  performance metrics:

  \begin{itemize}
  \tightlist
  \item
    λ vs.~Net Rating: \textbf{r = 0.76}
  \item
    λ vs.~Offensive Rating: \textbf{r = 0.76}
  \item
    λ vs.~Defensive Rating: \textbf{r = -0.17}
  \end{itemize}
\item
  \textbf{Information already captured:} The Bradley-Terry λ parameters
  encode team quality through win/loss patterns against opponents of
  varying strength. Teams that score more points (high offensive rating)
  naturally win more games, which increases their λ. Adding explicit
  offensive/defensive covariates provides no new information.
\item
  \textbf{Prediction mechanism:} Tournament predictions use
  \texttt{P(win)\ =\ 1/(1\ +\ exp(-(λ\_i\ -\ λ\_j)))}. Even though
  covariate models alter absolute λ values, the \textbf{relative
  differences} between teams (what matters for predictions) remain
  nearly identical.
\end{enumerate}

\textbf{Model Selection Justification:}

We use the \textbf{Base Model} (\texttt{team\ +\ home\_advantage})
because:

\begin{itemize}
\tightlist
\item
  ✓ \textbf{Same predictive accuracy} as complex models (69.89\% on test
  set)
\item
  ✓ \textbf{Same log-loss} (0.5779) and calibration (MSE = 0.0092)
\item
  ✓ \textbf{Simpler and more interpretable} (fewer parameters)
\item
  ✓ \textbf{Follows Occam's Razor:} Prefer simpler models when
  performance is equal
\item
  ✓ \textbf{More robust:} Fewer parameters reduce overfitting risk
\end{itemize}

This finding aligns with Bradley-Terry theory: team fixed effects
naturally absorb variance explained by team-level covariates when those
covariates are derived from the same game outcomes used to estimate team
strengths.
...
\subsubsection{⚠️ Critical Limitation: Synthetic
Seeding}\label{critical-limitation-synthetic-seeding}

This analysis uses \textbf{artificially generated tournament seeds}, not
actual NCAA tournament data. This is a fundamental limitation that
affects how results should be interpreted.

\textbf{What We Did:} - Ranked all teams by their Bradley-Terry strength
(λ) aggregated across 5 seasons (2019, 2021-2024) - Assigned seeds 1-16
to the top 64 teams across four regions based purely on team strength -
Simulated tournaments using these model-based seeds

\textbf{What We Did NOT Do:} - Use actual NCAA selection committee seeds
from real tournaments - Account for conference tournament results,
geographic placement, or selection committee subjective decisions -
Analyze historical tournament outcomes with real brackets

\textbf{Why This Matters:} - Our results answer: ``What if seeds
perfectly matched team strength?''---not ``What happened in real NCAA
tournaments?'' - Real committee seeding may over-seed or under-seed
teams based on factors beyond pure performance - Seed-specific
probabilities (e.g., ``8-seeds have 12\% chance of Sweet 16'') refer to
\textbf{strength-equivalent 8-seeds}, not actual committee-assigned
8-seeds - Real bracket matchups would differ from our simulated
structure

\textbf{Advantages of This Approach:} - Strength-based seeding removes
committee bias and politics - Accounts for opponent-adjusted strength
(strength of schedule), not just win percentage - Provides ``ground
truth'' baseline of what tournament outcomes would look like with
perfect seeding - More predictive than committee seeding in some cases
(e.g., identifying over/under-seeded teams)

\textbf{To Use This Model on Real Tournaments:} - Replace synthetic
seeds with actual NCAA bracket assignments - Re-run simulations using
committee-assigned matchups - Compare predictions against actual
outcomes for validation

\textbf{Interpretation:} Results represent \textbf{strength-based
projections} showing expected outcomes under optimal seeding, not
historical tournament analysis. See Section 4.3 for detailed discussion
of this limitation and its implications.
...
\textbf{We expect approximately 2.21 of the 8-12 seeds to advance to the
Sweet 16} in a typical tournament.

\begin{itemize}
\tightlist
\item
  \textbf{Simulation mean:} 2.21 teams (from 5,000 simulations)
\item
...
\item
  ...
  (typically 1-3 seeds)
\item
  Only about \textbf{1 in 53 teams (1.9\%)} that reach the Sweet 16 go
  on to win the championship
\item
  Nearly half (45\%) reach the Elite 8, showing they remain competitive
...
\textbf{Note:} The following comparison uses actual NCAA tournament
outcomes (2023-2024) against our strength-based model predictions. While
our model uses synthetic seeds, the observed pattern of zero mid-tier
seeds advancing is a real historical fact that we can contextualize.

Our model predicts approximately \textbf{2.21 teams seeded 8-12} should
reach the Sweet 16 on average. However, in both 2023 and 2024,
\textbf{zero 8-12 seeds} (as assigned by the NCAA selection committee)
...
\tightlist
\item
  Expected number of 8-12 seeds in Sweet 16: \textbf{2.21} (simulation
  mean)
\item
  Probability of zero 8-12 seeds reaching Sweet 16: \textbf{6.84\%}
\item
  \textbf{Interpretation:} About a \textbf{1-in-15 chance}---genuinely
  unusual
\end{itemize}
...
approximately \textbf{0.47\%} (6.84\% × 6.84\%), or about
\textbf{1-in-214}. This is genuinely rare and represents a notable
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Expected Sweet 16 Advancement:} Approximately 2.21 teams
...
\item
  ...
  \end{itemize}
\item
  \textbf{Model Insights:} Simulation-based estimates (2.21 teams) are
...
\item
  ...
  8-12 seeds in Sweet 16) has 6.84\% probability---genuinely unusual,
  occurring about once every 15 years. Back-to-back occurrence is
  extremely rare (0.47\%, or 1-in-214)
\end{enumerate}
...
\item
  ...
  about once every 15 years. Back-to-back ``chalk'' tournaments (like
  2023-2024) are extremely rare (1-in-214)
\item
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Synthetic Seeding (Critical Limitation):} This analysis uses
  \textbf{artificially generated seeds} based on Bradley-Terry λ values,
  \textbf{not actual NCAA tournament seeds}. This is a fundamental
  ...
  \tightlist
  \item
    The NCAA selection committee uses factors beyond raw team strength:
    strength of schedule, conference tournament performance,
    head-to-head records, geographic considerations, and subjective
    evaluation
  \item
    Real brackets may have ``overseeded'' or ``underseeded'' teams
    relative to their true strength
  \item
    Our seed-specific probabilities (e.g., ``8-seeds have X\% chance'')
    represent \textbf{strength-equivalent seeds}, not actual NCAA
    seeding decisions
  \item
    Real tournament matchups would differ from our simulated bracket
    structure
  \end{itemize}
  ...
  \tightlist
  \item
    Our findings about ``how many 8-12 seeds reach Sweet 16'' are based
    on teams that \emph{performed at the 8-12 seed strength level}, not
    teams that the selection committee actually seeded 8-12
  \item
    The strength-based approach may actually be more predictive than
    committee seeding (which can include political factors), but it's
    not the same as analyzing real historical tournaments
  \item
    To apply this model to an actual tournament, you would need to use
    the real NCAA bracket and seeds, not our synthetic ones
  \end{itemize}

  \textbf{Interpretation:} Results should be viewed as
  \textbf{strength-based projections} showing what would happen if seeds
  perfectly matched team quality, rather than predictions of specific
  NCAA tournaments with committee-assigned seeds.
\item
  \textbf{Multi-Season Data Aggregation:} We aggregate Bradley-Terry
  estimates across 5 seasons (2019, 2021-2024). While this provides more
  robust estimates, it means:
  ...
  \item
    Roster changes, coaching changes, and program trajectory are not
    captured
  \item
    Recent performance is weighted equally with older performance
  \item
    The ``2024 tournament'' we simulate uses aggregated multi-year data,
    not just 2024 season performance
  \end{itemize}
\item
  ...
  \item
    Travel, rest, and scheduling in tournaments differ from regular
    season
  \end{itemize}
\end{enumerate}
...
\item
  ...
  1-point win the same as a 30-point blowout. We tested
  margin-of-victory (MOV) extensions but found they provided no
  improvement:

  \begin{itemize}
  \tightlist
  \item
    MOV as game-level weights: AIC increased by \textasciitilde4000-4500
    (much worse)
  \item
    MOV as team-level covariate: AIC identical to base model (Δ = 0.0)
  \item
    Reason: Team strength (λ) already captures dominance through win
    patterns; MOV is redundant (r = 0.79 correlation)
  \end{itemize}
\item
  ...
  \tightlist
  \item
    Home court advantage in regular season (included in game-level model
    variant)
  \item
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{Alternative Performance Metrics:} While MOV,
  offensive/defensive ratings proved redundant with λ, other metrics
  might add value:
...
\item
  \textbf{Context-Specific Models:}

  \begin{itemize}
  \tightlist
  \item
    Game-level home advantage already implemented and validates
    neutral-site assumption
  \item
...
\tightlist
\item
  \textbf{Approximately 2.21 of the 8-12 seeds should advance to the
  Sweet 16} in a typical tournament (range: 0-7, typically 1-3 teams)
\item
  \textbf{An 8-12 seed that reaches the Sweet 16 has a 15.3\%
  probability of reaching the Final Four} and a 1.9\% probability of
  winning the championship (about 1-in-53)
\end{enumerate}
...
\item
  ...
  8-12 seeds in the Sweet 16 has a 6.84\% probability under our
  model---genuinely unusual, occurring about once every 15 years. The
  back-to-back occurrence is extremely rare (0.47\%, or 1-in-214).
\item
  \textbf{Methodological insight:} Simulation-based estimates (2.21
...
variation---it was a genuinely rare event (1-in-214) that demonstrated
...
\emph{Report generated: 2025-11-03}
